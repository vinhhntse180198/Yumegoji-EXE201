...
export async function fetchAdminGames() {
  const { data } = await gameApi.fetchAdminGames();
  return Array.isArray(data) ? data : [];
}

export async function createAdminGame(body) {
  const { data } = await gameApi.createAdminGame(body);
  return data ?? null;
}

export async function deleteAdminGame(gameId) {
  await gameApi.deleteAdminGame(gameId);
}
...
export async function endGameSession(sessionId) {
  ...
  const { data } = await gameApi.endSession(id);
  return data;
}
...
export async function fetchLeaderboard(q = {}) {
  const params = {};
  if (q.gameSlug) params.gameSlug = q.gameSlug;
  if (q.period) params.period = q.period;
  if (q.sortBy) params.sortBy = q.sortBy;
  if (q.levelId != null) params.levelId = String(q.levelId);
  if (q.friendsOnly) params.friendsOnly = 'true';
  const { data } = await gameApi.getLeaderboard({ params });
  return Array.isArray(data) ? data : [];
}

export async function fetchMyAchievements() {
  const { data } = await gameApi.fetchMyAchievements();
  return Array.isArray(data) ? data : [];
}
...
export async function fetchExpLeaderboard(limit = 10) {
  ...
  const { data } = await gameApi.fetchExpLeaderboard({ params: { limit: n } });
  return Array.isArray(data) ? data : [];
}
...
export async function purchaseGamePowerUp(body) {
  const { data } = await gameApi.purchasePowerUp({
...
export async function completeKanjiMemoryRewards(body) {
  const { data } = await gameApi.completeKanjiMemory({
...
export async function createPvpRoom(body) {
  const { data } = await gameApi.createPvpRoom({
    gameSlug: body.gameSlug,
...
export async function joinPvpRoom(body) {
  const { data } = await gameApi.joinPvpRoom({ roomCode: body.roomCode });
  return data;
}
